...
import { Clock, AlertTriangle, ShieldCheck, RefreshCw, Download, ChevronDown } from "lucide-react"
...
function toUtc7DateRange(range?: { from: Date; to: Date }) {
  ...
  // Format as ISO datetime strings with full time precision (hour:minute:second)
  // This matches the dashboard utama behavior for consistent filtering
  const result = {
...
function computeMTTD(alert: AlertItem, source: string): number | null {
  ...
  if (sourceNormalized.includes("stellar")) {
    ...
    if (userAction?.alert_to_first !== undefined && userAction?.alert_to_first !== null && userAction?.alert_to_first > 0) {
      ...
      console.log('[MTTD DEBUG] Using Priority 1 (alert_to_first):', {
        alertId: alert.id,
        alert_to_first: userAction.alert_to_first,
        result: formatMetric(userAction.alert_to_first)
      })
    ...
    // This is more reliable than updatedAt because it represents actual first action/assignment
    if (userAction?.history && Array.isArray(userAction.history)) {
      ...
      if (assigneeAction?.action_time) {
        ...
        const result = formatMetric(mttdMs)
        console.log('[MTTD DEBUG] Using Priority 2 (Event assignee changed to):', {
          alertId: alert.id,
          alertTime: new Date(alertTime).toISOString(),
          actionTime: new Date(actionTime).toISOString(),
          mttdMs,
          result
        })
        return result
    ...
    if (closedTime && alertTime) {
      ...
      if (closedTimeMs > alertTime) {
        ...
        const result = formatMetric(mttdMs)
        console.log('[MTTD DEBUG] Using Fallback 1 (closed_time):', {
          alertId: alert.id,
          alertTime: new Date(alertTime).toISOString(),
          closedTimeMs: new Date(closedTimeMs).toISOString(),
          mttdMs,
          result
        })
        return result
    ...
    if (userAction?.history && Array.isArray(userAction.history) && userAction.history.length > 0) {
      ...
      if (firstAction?.action_time) {
        ...
        const mttdMs = computeMetricMs(alertTime, actionTime)
        const result = formatMetric(mttdMs)
        console.log('[MTTD DEBUG] Using Fallback 2 (first action in history):', {
          alertId: alert.id,
          alertTime: new Date(alertTime).toISOString(),
          actionTime: new Date(actionTime).toISOString(),
          mttdMs,
    ...
    console.log('[MTTD DEBUG] No MTTD calculated (returning null):', {
      alertId: alert.id,
      alertTime: new Date(alertTime).toISOString(),
      hasUpdatedAt: !!alert.updatedAt,
      hasAlert_to_first: !! (userAction?.alert_to_first),
      hasUserAction: !!userAction,
      hasHistory: !!userAction?.history?.length,
      hasClosedTime: !!closedTime
    })
    return null
  }

  // QRadar: MTTD = offense start_time → first action
  // Priority 1: last_assigned_user_time (when user assigned)
  // Priority 2: first note/comment time
  if (sourceNormalized.includes("qradar")) {
    const qradarData = alert.metadata?.qradar as any
    if (!qradarData) {
      console.log('[MTTD DEBUG] QRadar: No qradar metadata', { alertId: alert.id })
      return null
    }

    // Get offense start time
    let offenseTsMs: number | null = null
    const startTime = qradarData.start_time
    
    if (typeof startTime === "number") {
      offenseTsMs = startTime > 1000000000000 ? startTime : startTime * 1000
    } else if (typeof startTime === "string") {
      offenseTsMs = new Date(startTime).getTime()
    }

    if (!offenseTsMs || !Number.isFinite(offenseTsMs)) {
      console.log('[MTTD DEBUG] QRadar: Invalid start_time', { alertId: alert.id, startTime })
      return null
    }

    // Priority 1: last_assigned_user_time (when user was assigned)
    const lastAssignedTime = qradarData.last_assigned_user_time
    if (lastAssignedTime !== undefined && lastAssignedTime !== null) {
      ...
      } else if (typeof lastAssignedTime === "string") {
      ...
      if (Number.isFinite(assignedTimeMs) && assignedTimeMs >= offenseTsMs) {
        const mttdMs = computeMetricMs(offenseTsMs, assignedTimeMs)
        if (mttdMs !== null) {
          const result = formatMetric(mttdMs)
          console.log('[MTTD DEBUG] QRadar Priority 1 - Using last_assigned_user_time:', {
            alertId: alert.id,
            offenseTime: new Date(offenseTsMs).toISOString(),
            assignedTime: new Date(assignedTimeMs).toISOString(),
            mttdMs,
            result
          })
          return result
    ...
    // Priority 2: First note/comment time from QRadar metadata
    const notes = qradarData.notes as any[] | undefined
    if (notes && Array.isArray(notes) && notes.length > 0) {
      // Sort by create_time to get earliest note
      ...
      if (firstNote?.create_time) {
        ...
        if (Number.isFinite(noteTimeMs) && noteTimeMs >= offenseTsMs) {
          const mttdMs = computeMetricMs(offenseTsMs, noteTimeMs)
          if (mttdMs !== null) {
            const result = formatMetric(mttdMs)
            console.log('[MTTD DEBUG] QRadar Priority 2 - Using first note:', {
              alertId: alert.id,
              offenseTime: new Date(offenseTsMs).toISOString(),
              firstNoteTime: new Date(noteTimeMs).toISOString(),
              mttdMs,
              result
            })
            return result
    ...
    console.log('[MTTD DEBUG] QRadar - No MTTD (no assignment or notes)', {
      alertId: alert.id,
      hasLastAssignedTime: lastAssignedTime !== undefined,
...
export default function SlaDashboardPage() {
  ...
  // Helper to filter items by selected integrations
  const filterBySelectedIntegrations = (items: any[]): any[] => {
    if (isAllIntegrations || selectedCount === 0) return items
    
    const selectedIds = integrationFilter.filter(f => f !== "all")
    return items.filter(item => {
      const itemIntegrationId = item.integrationId || item.integration?.id
      return selectedIds.includes(itemIntegrationId)
    })
  }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const selectedIntegrationIds = isAllIntegrations 
        ...
        : integrationFilter.filter(f => f !== "all")

      console.log('[SLA] fetchData started:', {
        scope,
        isAllIntegrations,
        ...
        for (const integId of integrationIds) {
          ...
          if (scope === "all" || scope === "alerts") {
            try {
              ...
              console.log(`[SLA] Fetched ${alertsData.length} alerts for integration ${integId}`)
              allAlerts = [...allAlerts, ...alertsData]
            } catch (err) {
              console.error(`[SLA] Failed to fetch alerts for integration ${integId}:`, err)
            }
          }
          ...
          if (scope === "all" || scope === "tickets") {
            try {
              ...
              console.error(`[SLA] Failed to fetch cases for integration ${integId}:`, err)
            }
          }
        }
      ...
      console.log("[SLA] Alerts fetched:", allAlerts.length)
      if (allAlerts.length > 0) {
        console.log("[SLA] Sample alert:", JSON.stringify(allAlerts[0], null, 2))
      }
      ...
      if (scope === "all" || scope === "tickets") {
        try {
          ...
          if (range) {
            wazuhParams.append("from_date", range.from)
            wazuhParams.append("to_date", range.to)
          } else {
            wazuhParams.append("time_range", "7d")
          }
          wazuhParams.append("limit", "10000")

          const wazuhUrl = `/api/wazuh/cases?${wazuhParams}`
          console.log("[SLA] Wazuh fetch URL:", wazuhUrl)
          
          const resWazuh = await fetch(wazuhUrl)
          console.log("[SLA] Wazuh response status:", resWazuh.status)
          
          const dataWazuh = await resWazuh.json()
          console.log("[SLA] Wazuh raw response:", dataWazuh)
          
          const wazuhCases = dataWazuh.cases || dataWazuh.data || []
          console.log("[SLA] Wazuh extracted cases:", wazuhCases.length)
          ...
          const wazuhCasesTransformed = wazuhCases
            .filter((c: any) => {
              ...
              if (!integration) {
                console.log(`[SLA] Warning: Wazuh case ${c.id} has unknown integrationId ${c.integrationId}`)
                return false
              }
              return true
            })
          ...
          if (wazuhCasesTransformed.length > 0) {
            ...
            console.log("[SLA] Wazuh cases added:", wazuhCasesTransformed.length)
          } else {
            finalCases = allCases
          }

          console.log("[SLA] Wazuh cases fetched from API:", wazuhCases.length)
        } catch (wazuhErr) {
          console.error("[SLA] Failed to fetch Wazuh cases:", wazuhErr)
        }
        ...
        try {
          ...
          if (range) {
            qradarParams.append("from_date", range.from)
            qradarParams.append("to_date", range.to)
          } else {
            qradarParams.append("time_range", "7d")
          }
          qradarParams.append("limit", "10000")

          const qradarUrl = `/api/qradar/cases?${qradarParams}`
          console.log("[SLA] QRadar fetch URL:", qradarUrl)
          
          const resQRadar = await fetch(qradarUrl)
          console.log("[SLA] QRadar response status:", resQRadar.status)
          
          const dataQRadar = await resQRadar.json()
          console.log("[SLA] QRadar raw response:", dataQRadar)
          
          const qradarCases = dataQRadar.cases || dataQRadar.data || []
          console.log("[SLA] QRadar extracted cases:", qradarCases.length)
          ...
          const qradarCasesTransformed = qradarCases
            .filter((c: any) => {
              ...
              const integration = integrations.find((i) => i.id === c.integrationId)
              if (!integration) {
                console.log(`[SLA] Warning: QRadar case ${c.id} has unknown integrationId ${c.integrationId}`)
                return false
              }
              return true
            })
          ...
          if (qradarCasesTransformed.length > 0) {
            ...
            console.log("[SLA] QRadar cases added:", qradarCasesTransformed.length)
          }

          console.log("[SLA] QRadar cases fetched from API:", qradarCases.length)
        } catch (qradarErr) {
          console.error("[SLA] Failed to fetch QRadar cases:", qradarErr)
        }
      }

      setCases(finalCases)
      console.log("[SLA] Final cases set:", finalCases.length, "total cases")
    } catch (err) {
  ...
  const slaRows = useMemo(() => {
    ...
    const caseRows = cases.map((c) => {
      ...
      // For all cases: use MTTR (Mean Time To Response)
      // QRadar: from oldest alert to case creation
      // Others: from case creation to resolution
      const sourceNormalized = source?.toLowerCase() || ""
      const metric = sourceNormalized.includes("qradar") ? c.mttrMinutes : computeMTTR(c)
      
      // For tickets, DO NOT use severityBasedOnAnalysis (per user request)
      const sev = getSeverityValue(c.severity || undefined, null)
      
      const threshold = severityThresholdMinutes[sev]
      const pass = metric === null ? false : metric <= threshold
      
      // Debug log for first few cases
      if (cases.indexOf(c) < 3) {
        console.log('[SLA] Ticket metric calculation:', {
          id: c.id,
          source,
          status: c.status,
          severity: c.severity,
          ...
          isQRadar: sourceNormalized.includes("qradar"),
          metric,
          threshold,
        })
      }
            ...
            <Popover open={integrationPopoverOpen} onOpenChange={setIntegrationPopoverOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-between">
                  <span className="truncate">{getIntegrationDisplayLabel()}</span>

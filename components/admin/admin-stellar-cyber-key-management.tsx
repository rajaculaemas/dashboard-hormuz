...
interface AdminStellarCyberKeyManagementProps {
  userId: string;
  userName: string;
}
...
export function AdminStellarCyberKeyManagement({ 
  userId, 
  userName 
}: AdminStellarCyberKeyManagementProps) {
  ...
  const fetchCredentials = async () => {
    try {
      setLoading(true);
      console.log(`[AdminStellarKey] Fetching credentials for userId: ${userId}`);
      ...
      if (integrationsResponse.ok) {
        ...
        const hostsArray = Array.from(uniqueHosts).sort();
        console.log(`[AdminStellarKey] Found hosts:`, hostsArray);
        setHosts(hostsArray);
      }

      // Fetch user's Stellar Cyber credentials by host
      const credentialsResponse = await fetch(`/api/users/${userId}/stellar-key/hosts?t=${Date.now()}`);
      console.log(`[AdminStellarKey] Credentials endpoint status:`, credentialsResponse.status);
      
      if (credentialsResponse.ok) {
        const data = await credentialsResponse.json();
        console.log(`[AdminStellarKey] Credentials response:`, data);
        setCredentials(data.credentials || []);
      } else if (credentialsResponse.status === 403) {
        console.warn(`[AdminStellarKey] Permission denied for userId: ${userId}`);
        toast({
          title: 'Permission Denied',
          description: 'You do not have permission to view this user\'s Stellar keys',
          variant: 'destructive',
        });
      } else {
        const errorData = await credentialsResponse.json();
        console.error(`[AdminStellarKey] Error response:`, errorData);
        toast({
          title: 'Error',
          description: `Failed to load Stellar keys: ${errorData.error || 'Unknown error'}`,
          variant: 'destructive',
        });
      }
    } catch (error) {
      console.error('[AdminStellarKey] Error fetching Stellar Cyber data:', error);
      toast({
        title: 'Error',
  ...
  useEffect(() => {
    console.log(`[AdminStellarKey] useEffect triggered for userId: ${userId}`);
    fetchCredentials();
  }, [userId, toast]);

  // Refetch credentials when dialog is opened
  useEffect(() => {
    console.log(`[AdminStellarKey] Dialog opened state:`, openDialog);
    if (openDialog) {
      fetchCredentials();
    }
  }, [openDialog, userId, toast]);
  ...
  const handleSaveKey = async () => {
    ...
    try {
      setSaving(true);
      const response = await fetch(`/api/users/${userId}/stellar-key/hosts`, {
        method: 'POST',
  ...
  const handleDeleteKey = async (host: string) => {
    ...
    if (!confirm(`Delete API key for ${hostName}? This user will no longer be able to update alerts from this host.`)) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `/api/users/${userId}/stellar-key/hosts?host=${encodeURIComponent(host)}`,
        { method: 'DELETE' }
      );
    ...
    <Card className="mt-4">
      <CardHeader>
        <div className="flex justify-between items-start">
          <div>
            <CardTitle>Stellar Cyber API Keys for {userName}</CardTitle>
            <CardDescription>
              Manage API keys for each Stellar Cyber host
            </CardDescription>
          </div>
        </div>
      </CardHeader>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <Input
                              id="apiKey"
                              type="password"
                              placeholder="Paste Stellar Cyber API key"
                              value={apiKey}
